...
export default function UsersPage() {
  ...
  const roleIcon = (role) => {
    const r = ROLE_OPTIONS.find(o => o.key === role)
    return r?.icon || Users
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <div className="px-6 pt-6">
        <div className="max-w-7xl mx-auto">
          ...
          <h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-900">People & Roles</h1>
          <p className="text-sm text-slate-600">Browse users by professional roles and details</p>
        </div>
      </div>

      {/* Filters */}
      <div className="sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-gray-200 shadow-sm mt-4">
        <div className="max-w-7xl mx-auto px-6 py-5 flex flex-col lg:flex-row gap-4 items-center justify-between">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-white border border-gray-200 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200"
            />
          </div>
          ...
          <div className="flex flex-wrap gap-3 items-center">
            <select
              ...
              className="px-4 py-3 bg-white border border-gray-200 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200"
            >
              {ROLE_OPTIONS.map(r => (
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="mb-6 text-sm text-gray-600 font-medium">
          Showing <span className="font-semibold">{visibleUsers.length}</span> of <span className="font-semibold">{users.length}</span> users
        </div>
        ...
        <div className="space-y-12">
            ...
            return (
              <div key={role}>
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="text-xl font-semibold text-slate-800">{role}</h2>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {sorted.map(u => (
                    <Card
                      key={u.id}
                      className="p-6 bg-white border border-gray-200 rounded-2xl shadow-sm hover:shadow-lg transform hover:-translate-y-1 transition-all duration-300"
                    >
                      <div className="flex items-start justify-between">
                        <div>
                          <div className="text-lg font-semibold text-slate-900">{u.name || u.userId}</div>
                          <div className="text-sm text-gray-500">{u.email || u.phone || '—'}</div>
                        </div>
                        <Badge color={u.status === 'Active' ? 'green' : 'amber'} className="text-sm">{u.status}</Badge>
                      </div>

                      <div className="mt-5 grid grid-cols-2 gap-3 text-sm">
                        <div className="p-3 bg-white border border-gray-200 rounded-xl shadow-sm">
                          <div className="text-[11px] font-medium text-gray-500">User ID</div>
          ...
          {!visibleUsers.length && (
            <div className="text-center py-16">
              <div className="bg-white/60 backdrop-blur-md rounded-2xl p-12 max-w-md mx-auto shadow-md">
